...
describe('Email Service', () => {
	...
	it('sendMessage - should reject in case of no email adress to be send to', done => {
		emailService
			.sendMessage(null, { mock: 'any' })
			.then(() => {
				done(
					new Error(
						'sendMessage resolved, but it should reject because of no email adress to send',
					),
				)
			})
			.catch(err => {
	...
	it('sendMessage - should reject in case of no messageParams in second argument', done => {
		emailService
			.sendMessage('test@test-env.com', null)
			.then(() => {
	...
	it('sendMessage - should resolve and send email', done => {
		emailService
			.sendMessage('test@test-env.com', { mock: 'any' })
			.then(() => {
				done()
			})
			.catch(() => {
